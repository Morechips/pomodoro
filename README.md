...
3. **方便他人使用**：让习惯使用番茄钟（Catime）的用户可以快速下载并上手。  
4. **方便维护扩展**：记录功能边界与设计取舍，便于后续优化。
...
1. **更纯粹，不打扰学习**  
   配合 Catime 使用：专注阶段不弹窗、不桌宠、不占注意力；仅在倒计时结束时打开网页，提醒你“该收尾了”。
...
├─ pomodoro_replay.html        # 复刻/重播页面（用于复用/脚本改写目标之一）
├─ src/
...
- **90 分钟**：贴合计算机专业学生与写代码人群——写代码常需要更长专注才能产出，且不应频繁打断心流。
...
- **可选**：用 `open.bat` 启动（如果需要网页的自动播放提示音的话，但不能保证一定可以自动播放）。
...
4. 指向本项目的 `index.html`：
   - 推荐：选择本地文件（`index.html` 的路径）
   - 或使用本地服务时填写 `http://localhost/...`
...
<p align="center"><sub><b>小习惯创造大价值</b></sub></p>
